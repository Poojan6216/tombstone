...
def main() -> int:
    ...
    problems: list[str] = []
    r = json.loads((RESULTS / "residue-latest.json").read_text()) if (RESULTS / "residue-latest.json").is_file() else None
    if r:
        for c in r["cells"]:
            ...
            if not t:
                continue
            if c["baseline"] == "B4" and c["physical_checked"] and c["physical_residue_rate"] > t.get("max_physical_residue", 0.0):
                problems.append(f"{key}: physical residue {c['physical_residue_rate']:.3f} > {t['max_physical_residue']}")
            if c["logical_exclusion_rate"] < t.get("min_logical_exclusion", 0.0):
                problems.append(f"{key}: logical exclusion {c['logical_exclusion_rate']:.3f} < {t['min_logical_exclusion']}")
            if c["recall_at_5_after"] < c["recall_at_5_before"] - t.get("max_recall_drop", 1.0):
                problems.append(f"{key}: recall@5 dropped {c['recall_at_5_before']:.3f} → {c['recall_at_5_after']:.3f}")
    u = json.loads((RESULTS / "unlearn-latest.json").read_text()) if (RESULTS / "unlearn-latest.json").is_file() else None
    if u:
        for m in u["methods"]:
            ...
            if m["canary_rate"] > t.get("max_canary_rate", 1.0):
                problems.append(f"{m['name']}: canary rate {m['canary_rate']:.3f} > {t['max_canary_rate']}")
    for p in problems:
