...
|---|---|---|---|---|
| Chroma (embedded) | metadata flag + filter on every wrapper query path | scan `chroma.sqlite3` (queue, metadata, free pages) and every HNSW segment file for the fingerprint and the id | delete → rewrite the collection from survivors into a fresh segment → zero the HNSW slots past `cur_element_count` (Chroma persists the whole malloc'd buffer, uninitialised heap included; on Linux that heap has held the just-erased vectors) → purge the embeddings queue → `VACUUM` → remove orphan segment dirs | persist dir not readable → `{LOGICAL}` |
| Pinecone (managed) | metadata flag + `$eq` filter on every query path, after waiting for the update to become visible | **none** — no persist directory, no snapshot, no file-read API; `probe_physical` raises rather than reporting absence | `index.delete(ids=…)`, then wait until the service stops returning them; the freed bytes are not observable from here | always managed → `{LOGICAL}`, receipt says `UNVERIFIED-managed` |
