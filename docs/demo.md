# Demos

Generated by `uv run python bench/demo.py --subject S-0042` on 2026-09-11; every block below is captured from that run. Subject ids are shown as HMACs; the raw id never reaches disk.

## Demo 1 — `delete()` is a lie

The app deleted the way everyone deletes: `vectorstore.delete(ids=[...])` on both indexes and the source rows dropped. Then:

```text
$ tombstone verify --subject S-0042 --after-native-delete
after native delete()
subject: hmac:f675…1d04   (raw id never logged)
artifacts descending from this subject: 23

  source  ×3 docs                   HIDDEN     logical PASS, physical FAIL
                                               bytes for 3/3 ids located in docs.sqlite
  chunk   ×4 docs                   HIDDEN     logical PASS, physical FAIL
                                               bytes for 4/4 ids located in docs.sqlite
  embed   ×4 chroma:kb-v2           HIDDEN     logical PASS, physical FAIL
                                               bytes for 4/4 ids located in chroma.sqlite3 + data_level0.bin
  embed   ×4 pgvector:kb-v1         HIDDEN     logical PASS, physical FAIL
                                               bytes for 4/4 ids located in documents_embedding_hnsw + pg_toast.pg_toast_17061
  cache   ×1 exact-cache            PRESENT    cached answer still returned
  cache   ×1 semantic-cache         PRESENT    cached answer still returned
  train   ×4 ft-dataset             PRESENT    still retrievable by id
  adapter ×2 lora/support-v3        GONE       no extraction prompts

verdict: NOT ERASED.  21/23 artifacts still hold recoverable content.
exit 2
```

**21 of 23 artifacts (91.3%) are still recoverable** after the app's own `delete()` returned success.

## Demo 2 — the cascade, and what it can honestly claim

```text
$ tombstone erase --trace 6NBXMGHHX2TWZB986D7CXKMW3R --reason dsr-2026-0912 --confirm --semantic
phase 1 suppress    23/23 artifacts tombstoned         (retrieval filter active)
phase 2 reclaim
  chroma:kb-v2      compact + rewrite segment          semantic RESIDUAL          (retrieval-context drift 0.0186 exceeds same-cluster control 0.0167 (CI)
  docs              DELETE + VACUUM                    physical VERIFIED          (bytes absent, 7/7)
  exact-cache       DELETE + VACUUM                    physical VERIFIED          (bytes absent, 1/1)
  ft-dataset        drop row, re-hash manifest         physical VERIFIED          (bytes absent, 4/4)
  lora/support-v3   approximate unlearn (npo, 40 steps model VERIFIED             canary 0/4
  pgvector:kb-v1    DELETE + REINDEX + VACUUM FULL     physical VERIFIED          (pgstattuple dead=0, bytes absent, 4/4)
  semantic-cache    invalidate + purge 0 neighbours    physical VERIFIED          (bytes absent, 1/1)
phase 3 verify
  semantic residue chroma:kb-v2   drift 0.019 vs same-cluster control 0.017   → above control
                   pgvector:kb-v1 drift 0.008 vs same-cluster control 0.008   → at control level

receipt: /Users/poojanpatel/Documents/Projects/tombstone/bench/_work/demo2/.tombstone/receipts/01M28AD1SWE5KSXHQ5BP6VQMKE.json   ed25519 signed   chain ok
  VERIFIED  19   UNVERIFIED 0   RESIDUAL 4   OUT_OF_SCOPE 3 (backups, WAL, provider-side embeddings)
  id 01M28AD1SWE5KSXHQ5BP6VQMKE   semantics v1
  A receipt is a record of what was done and checked. It is not a legal instrument.
exit 2
exit 2
```

`OUT_OF_SCOPE` is a count, and it is never zero, because there are always layers this tool cannot see. The receipt says so.

## Demo 3 — the honest one

Same subject, but the pgvector instance is reached through a role without file-read or maintenance rights (a managed database), the adapter was trained without sharding, and the subject is mentioned inside two other subjects' documents.

```text
$ tombstone erase --trace 7XRZ361ZYCP8X462WRCVGK80HG --reason dsr-2026-0913 --confirm
phase 1 suppress    23/23 artifacts tombstoned         (retrieval filter active)
phase 2 reclaim
  chroma:kb-v2      compact + rewrite segment          physical VERIFIED          (bytes absent, 4/4)
  docs              DELETE + VACUUM                    physical VERIFIED          (bytes absent, 7/7)
  exact-cache       DELETE + VACUUM                    physical VERIFIED          (bytes absent, 1/1)
  ft-dataset        drop row, re-hash manifest         physical VERIFIED          (bytes absent, 4/4)
  lora/support-v3   approximate unlearn (npo, 40 steps model VERIFIED             canary 0/4
  pgvector:kb-v1    DELETE only; REINDEX/VACUUM not pe physical UNVERIFIED-managed (physical UNVERIFIED-managed: this role cannot read relation files (pg_read_binary_file); g)
                    → run VACUUM FULL / REINDEX from an owner role, then `tombstone verify --receipt`
  semantic-cache    invalidate + purge 0 neighbours    physical VERIFIED          (bytes absent, 1/1)
phase 3 verify
  semantic residue  not measured
  third-party mentions: 2 documents owned by other subjects contain this subject's name
                    NOT erased (no lineage edge; would require editing other subjects' data). Listed for human review.

receipt: /Users/poojanpatel/Documents/Projects/tombstone/bench/_work/demo3/.tombstone/receipts/01M28BV4B8V8JV2WQ97VCDEG0M.json   ed25519 signed   chain ok
  VERIFIED  19   UNVERIFIED 4   RESIDUAL 0   OUT_OF_SCOPE 3 (backups, WAL, provider-side embeddings)   NEEDS_HUMAN 2
  id 01M28BV4B8V8JV2WQ97VCDEG0M   semantics v1
  A receipt is a record of what was done and checked. It is not a legal instrument.
exit 2
exit 2
```

A tool that prints "erasure complete" here would be lying to a regulator on the operator's behalf. Tombstone refuses.
